...
У каждой валюты есть поля Value (курс за номинал) и VunitRate (курс за одну
единицу). VunitRate — это «истинный» курс за единицу валюты: для валют с
Nominal=100 они различаются (см. JPY выше). При извлечении курса используйте
VunitRate, если это поле есть в словаре валюты, иначе — Value.

Напишите две функции:

get_currency_rate(data, code) -> float
    Возвращает курс валюты с заданным кодом CharCode как число float.
    Если валюта с таким кодом отсутствует — возвращает None.

get_all_rates(data) -> dict
    Возвращает словарь {код_валюты: курс_float} для всех валют,
    представленных в data.
"""
